...
ll first_true(ll l,ll r){ll ans=r; while(l<=r){double mid=l+(r-l)/2; if(check(mid)){ans=mid;r=mid-1;}else l=mid+1;}return ans;}
//Last True(For Maximum valid)
double last_true(double l, double r){double ans=l; while(r-l>eps){double mid=l+(r-l)/2;if(check(mid)){ans=mid;l=mid;}else r=mid;}return ans;}
